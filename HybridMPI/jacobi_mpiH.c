...
// We made error_in global to give it the scope it required for reductions
double error_in=0;

// The size of the scheduling chuncks 
#define CHUNK_SZ 4 // After trial and error tests, 4 seems like a good value but it does not affect it by much
...
int main(int argc, char **argv){
    ...
    double alpha, tol, relax, error = HUGE_VAL, *tmp, updateVal, err_sum=0;
    ...
    if( my_rank == 0 ){
        ...
        printHeader(2, n, m, alpha, relax, tol, mits, comm_sz);
    ...
    while (iterationCount < mits && error > tol ) {  
        ...
        y_off = f_index[0]+1;
        // Parallelize this for loop and use reduction to ensure the error is correct
        // Unfortunately We could not collapse this nested loop since it was not perfectly nested 
        #pragma omp for schedule(static, CHUNK_SZ) reduction(+:error_in) 
